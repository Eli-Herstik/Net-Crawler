...
from css_selectors import INTERACTIVE_SELECTORS, POPUP_CONTAINER_SELECTORS
...
class APIMapper:
    ...
    async def map_website(self) -> Dict[str, Any]:
        ...
        # Check DOM hash for the start page
        dom_hash = await self.dom_hasher.get_dom_hash(self.page)
        if dom_hash:
    ...
    async def _restore_page(self, page: Page, target_url: str) -> bool:
        """Go back or navigate to target_url to restore page state."""
    ...
    async def _explore_if_new(self, page: Page, url: str, depth: int):
        """If URL is new and DOM is unseen, explore it recursively."""
        if not self.navigator._should_follow_url(url):
            logger.debug("Skipping external/excluded URL: %s", url)
            return

        if url in self.navigator.visited_urls:
            return

        dom_hash = await self.dom_hasher.get_dom_hash(page)
        if dom_hash and self.dom_hasher.is_dom_seen(dom_hash):
            logger.debug("Skipping duplicate page (DOM hash match): %s", url)
            return

        if dom_hash:
            self.dom_hasher.mark_dom_seen(dom_hash)

        self.navigator.visited_urls.add(url)
    ...
    async def _explore_page(self, page: Page, depth: int):
        ...
        for i, element in enumerate(clickable_elements):
            ...
            if clicked:
                ...
                if current_url != base_url:
                    await self._explore_if_new(page, current_url, depth)
                    await self._restore_page(page, base_url)
                    ...
                    if href and href not in self.navigator.visited_urls:
                        if self.navigator._should_follow_url(href):
                            ...
                            if await self.navigator.navigate_to(page, href, depth + 1):
                                # Check DOM hash for the navigated page
                                dom_hash = await self.dom_hasher.get_dom_hash(page)
                                if dom_hash and self.dom_hasher.is_dom_seen(dom_hash):
                                    logger.debug("Skipping duplicate page (DOM hash match): %s", href)
                                else:
                                    if dom_hash:
                                        self.dom_hasher.mark_dom_seen(dom_hash)
                                    saved_clicks = self.navigator.clicks_on_current_page
